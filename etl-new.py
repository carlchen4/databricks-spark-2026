...
# summary current table and previous - remove backup - done 
...
# cnt for count, source scr, target tgt 
# row hash to be loaded in target table? - 
# use wiget for all para and saved into a workplace - 
# create dummy data based on schema in databricks - 
# validate versioning - 
# test error code and category - done 
...
class ETL:
  ...
  {"=" * 60}""")
    ...
    def print_summary(self, run_id: str) -> None:
        ...
        start_local = r['start_ts'].astimezone(tz_toronto) if r['start_ts'] else None
        end_local   = r['end_ts'].astimezone(tz_toronto) if r['end_ts'] else None
    ...
    def run_etl(self, partition_column: str, environment: str = "PROD", watermark_from: str = None, watermark_to: str = None) -> str:
        ...
        if reject_count > 0:
            ...
            df_good = df_good.withColumn(
                "amount",
                when(col("transaction_id") == df_good.collect()[1]["transaction_id"], lit(999.99))
            ...
            metrics.target_hash = str(current_batch_target
